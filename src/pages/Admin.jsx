...
const Field = ({ label, name, errors, children }) => (
  <div className="flex flex-col gap-1">
    <div className="relative">
      <label className="absolute -top-2.5 left-3 bg-white px-1 text-sm text-gray-500 z-10">{label}</label>
      {children}
    </div>
...
const AdminLogin = ({ onSwitch }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await adminApi.post('/admins/login', { email, password });
       console.log('Full login response:', response.data);  // ← ADD THIS
      ...
      setError(err.response?.data?.error?.msg || 'Invalid email or password');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <Field label="Password" name="password" errors={{}}>
          <div className="flex items-center border border-gray-400 rounded-lg px-4 py-3 gap-2 focus-within:border-[#0C850C] transition-colors">
            ...
            />
            <EyeToggle show={showPassword} onToggle={() => setShow(!showPassword)} />
          </div>
        </Field>
      ...
      <p className="text-sm text-gray-500 text-center">
        ...
        <button onClick={onSwitch} className="text-[#0C850C] font-medium underline">Sign Up</button>
      </p>
    </div>
  );
};
    ...
    <div className="flex flex-col gap-6 w-full">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <Field label="Email Address" name="email" errors={errors}>
          <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="admin@gorefresh.com"
            className="w-full border border-gray-400 rounded-lg px-4 py-3 text-sm outline-none focus:border-[#0C850C] transition-colors bg-transparent" />
        </Field>

        <Field label="Phone Number" name="phone" errors={errors}>
          <input
            type="tel" name="phone" value={form.phone} placeholder="+2348012345678"
            onChange={e => {
      ...
      <p className="text-sm text-gray-500 text-center">
        ...
        <button onClick={onSwitch} className="text-[#0C850C] font-medium underline">Sign In</button>
      </p>
    </div>
  );
};
